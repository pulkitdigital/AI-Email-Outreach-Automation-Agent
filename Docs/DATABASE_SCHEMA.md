...
> **Phase 3 note:** `generated_by` ended up `'pptxgenjs-template'` rather than an AI provider
> name — deck generation turned out to need no AI call at all (see
> `Docs/ARCHITECTURE.md` § 3). The column stays generic (`TEXT`, not a `CHECK`-constrained enum)
> so this didn't need a migration. `file_key`/`file_url` work the same regardless of storage
> backend — R2 in production, local disk in dev (see `Backend/src/storage/`); `file_url` falls
> back to this backend's own `GET /api/decks/:id/download` route when no R2 public URL is
> configured.
...
CREATE TABLE pitch_decks (
    ...
    generated_by        TEXT,                -- 'pptxgenjs-template' as of Phase 3 (see note above)
